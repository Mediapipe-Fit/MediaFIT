...
public class CameraSource {
   ...
   */
  private void setRotation(Camera camera, Camera.Parameters parameters, int cameraId) { // 카메라 각도조절
    ...
    int displayAngle;
    if (cameraInfo.facing == CameraInfo.CAMERA_FACING_FRONT) {  // 카메라 전면 사용
   ...
   */
  @SuppressLint("InlinedApi")
  private byte[] createPreviewBuffer(Size previewSize) { // 미리보기 화면 생성 -> 카메라 앱에서 우리가 사진찍기 전에 보는 화면
  ...
  private class CameraPreviewCallback implements Camera.PreviewCallback {
    @Override
    public void onPreviewFrame(byte[] data, Camera camera) { // 보여줄 크기 지정
